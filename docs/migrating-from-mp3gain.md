...
The `Max global_gain` / `Min global_gain` columns report the scanned range for MP3 and for AAC in MP4/M4A, matching what `-x` prints for the same file. They carry `-` when the file's gain fields cannot be scanned at all, which in practice means a raw ADTS `.aac` stream: mp3rgain cannot adjust those either. Up to 3.6.1 an unscanned file printed `255` / `0`, the pair the scanner starts its accumulators at, which is indistinguishable from a real full-range measurement ([#329](https://github.com/M-Igashi/mp3rgain/issues/329)).
